 ...
 */
router.get('/download/:fileKey', async (req: Request, res: Response) => {
  try {
 ...
 */
router.delete('/files/:fileKey', async (req: Request, res: Response) => {
  try {
    const fileKey = req.params.fileKey;
...
// Mount router on BOTH /api and / to handle all rewrite scenarios seamlessly
app.use('/api', router);
app.use('/', router);
...
app.use((err: any, _req: Request, res: Response, _next: any) => {
  ...
  res.status(500).json({
    success: false,
    error: err?.message || 'Server error encountered in LabDrop API.',
  });
});
